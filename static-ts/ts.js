(function(w) {
    ...
    w.gameInit = function() {
        createjs.Sound.registerSound({
            src: "./static-js/music/err.mp3",
            id: "err"
        });
        createjs.Sound.registerSound({
            src: "./static-js/music/end.mp3",
            id: "end"
        });
        createjs.Sound.registerSound({
            src: "./static-js/music/tap.mp3",
            id: "tap"
        });
        gameRestart();
    }
    ...
    w.refreshGameLayer = function(box, loop, offset) {
        ...
        for (let j = 0; j < box.children.length; j++) {
            ...
            if (i === j) {
                ...
                r.className += ' t' + (Math.floor(Math.random() * 1000) % 20+ 1);
                r.notEmpty = true;
    ...
    w.gameTapEvent = function(e) {
        ...
        if ((p.id === tar.id && tar.notEmpty) || (p.cell === 0 && x < blockSize) || (p.cell === 1 && x > blockSize && x < 2 *
            ...
            _gameBBListIndex++;
            _gameScore++;
            //document.getElementById(_ttreg).style.filter.invert(randomnumber);
